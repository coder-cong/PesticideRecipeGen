...
def compute_log_probs(model, is_ref, tokenizer, prompt_tokens, response, prompt_length, device):
    ...
    full_input_ids = torch.cat(
        [prompt_tokens.input_ids, response_tokens], dim=1).to(device)
...
def main(args):
    ...
    # 加载数据集
    dataset = read_gsmk8k(
        args.data_dir, sys_prompt=SYSTEM_PROMPT, split=args.split)
    data_loader = DataLoader(dataset, batch_size=1,
                             shuffle=True)  # 批次大小为1，因为我们生成多个答案
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lora_alpha", type=int,
                        default=32, help="LoRA alpha参数")
    parser.add_argument("--lora_dropout", type=float,
                        default=0.1, help="LoRA丢弃率")
